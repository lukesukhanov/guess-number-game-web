...
class PopUpRegisterButton {
	...
	static handleClick(event) {
		...
		const repeatedPasswordInput = registrationForm.elements["repeated-password"];
		const username = usernameInput.value.trim();
		const password = passwordInput.value;
		if (PopUpRegisterButton.validateInput(usernameInput, passwordInput, repeatedPasswordInput)) {
	...
	static validateInput(usernameInput, passwordInput, repeatedPasswordInput) {
		...
		if (password && password === repeatedPassword) {
			...
			repeatedPasswordInput.style.borderColor = POP_UP_FORM_DEFAULT_BORDER_COLOR;
		} else {
